...
func spawn_projectile(spawn_pos: Vector2, target_node, damage: int, speed: float, team_name: String, is_homing: bool = true, splash: float = 0.0, arc_height_grids: float = 0.0, emit_offset_y: float = 0.0, is_piercing: bool = false, max_range_px: float = 0.0, pierce_radius_px: float = 0.0, source_unit_id: String = "", source_hit_event_id: String = "") -> Node2D:
	...
	# 穿透箭（神箭游侠）按攻击动画方向发射：向上攻击（目标在上方）从占位符上方2.5格射出，
	# 向下攻击从占位符上方0.5格射出。基于目标 y 判断，Client 端 mirror 后方向自动正确。
	if is_piercing:
		var tp := BattlePathing.game_position_of(target_node) if target_node else spawn_pos
		# 向上攻击从上方2.5格射出，向下攻击从上方0.5格射出
...
## 返回: 生成的炮弹节点
func spawn_mortar_shell(spawn_pos: Vector2, target_node, damage: int, splash_px: float, speed_px: float, team_name: String, arc_grids: float, attack_ground: bool = true, attack_air: bool = true, impact_summon_unit_id: String = "", appearance: String = "stone") -> Node2D:
	var shell = MORTAR_SHELL_SCENE.instantiate()
	projectiles_root.add_child(shell)
	shell.setup_shell(spawn_pos, target_node, damage, splash_px, speed_px, team_name, arc_grids, attack_ground, attack_air, impact_summon_unit_id, appearance)
	...
	if NetworkManager.is_server():
		...
		_next_net_id += 1
		shell.name = net_name
		_rpc_spawn_mortar_shell.rpc(net_name, spawn_pos, target_pos, damage, splash_px, speed_px, team_name, arc_grids, impact_summon_unit_id, appearance)
	return shell
...
func _rpc_spawn_projectile(proj_name: String, spawn_pos: Vector2, target_pos: Vector2, dmg: int, spd: float, team_name: String, is_homing: bool, splash: float, arc_height_grids: float, emit_offset_y: float = 0.0, is_piercing: bool = false, max_range_px: float = 0.0, pierce_radius_px: float = 0.0, source_unit_id: String = "", source_hit_event_id: String = "") -> void:
	...
	# 穿透箭按攻击动画方向发射：向上攻击从上方2.5格射出，向下从上方0.5格射出
	if is_piercing:
		var grids := 2.5 if m_target.y < m_spawn.y else 0.5
		m_spawn.y -= grids * BattleConstants.CELL_SIZE
	# 发射点上移 emit_offset_y 对齐飞行高度（mirror 不改变"上=y减小"方向，两端都减）
	...
	# 穿透模式：client 端 setup 传 null target 导致方向未算，这里用镜像后的目标方向补算
	if is_piercing:
		projectile._fly_dir = (m_target - m_spawn).normalized()
	if arc_height_grids > 0.0:
		projectile.arc_height = arc_height_grids
...
@rpc("authority", "call_remote", "reliable")
func _rpc_spawn_mortar_shell(shell_name: String, spawn_pos: Vector2, target_pos: Vector2, dmg: int, splash_px: float, speed_px: float, team_name: String, arc_grids: float, impact_summon_unit_id: String = "", appearance: String = "stone") -> void:
	if NetworkManager.is_server():
		return
	...
	var local_team := "enemy" if team_name == "player" else "player"
	shell.setup_shell(m_spawn, null, dmg, splash_px, speed_px, local_team, arc_grids, true, true, impact_summon_unit_id, appearance)
