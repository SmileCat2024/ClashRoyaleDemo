...
# 依赖节点：Arena、UnitsRoot、TowersRoot、SpawnManager、SimpleEnemyAI
...
## 遍历 TowersRoot 下的所有塔节点，根据节点名称设置阵营和类型
...
func _select_hand_card(hand_index: int) -> void:
	...
	if not can_afford_card("player", card_id):
		print("[BattleManager] 能量不足:", card_id, "(需要", DataRegistry.get_card_data(card_id).get("cost", 0), "当前", player_energy, ")")
		return
	# 再次点击同一张 = 取消
...
func spend_energy(team_name: String, amount: int) -> void:
	if team_name == "player":
		...
		enemy_energy = max(0, enemy_energy - amount)
		SignalBus.energy_changed.emit("enemy", enemy_energy, max_energy)
...
func add_energy(team_name: String, amount: int) -> void:
	if team_name == "player":
		...
		SignalBus.energy_changed.emit("player", player_energy, max_energy)
		print("[Cheat] player energy +", amount, "->", player_energy)
	else:
		enemy_energy = min(max_energy, enemy_energy + amount)
		SignalBus.energy_changed.emit("enemy", enemy_energy, max_energy)
		print("[Cheat] enemy energy +", amount, "->", enemy_energy)
