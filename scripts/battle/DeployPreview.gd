...
## 预览圆半径（匹配 UnitBase body size 16px → radius 8）
...
func _check_valid() -> bool:
	if _arena == null or not _arena.has_method("is_player_deploy_position"):
		return false
	return _arena.is_player_deploy_position(_mouse_pos)
...
func _draw() -> void:
	...
	for offset in _offsets:
		var pos: Vector2 = _mouse_pos + offset
		draw_circle(pos, PREVIEW_RADIUS, fill)
		draw_arc(pos, PREVIEW_RADIUS, 0, TAU, 32, color, 1.5)
