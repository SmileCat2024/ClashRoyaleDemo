...
var source_unit_id: String = ""  ## 发射单位，用于仅播放该卡牌的专属命中音
var source_hit_event_id: String = ""  ## 可选的变体命中事件（如觉醒火枪手）
...
func setup(spawn_pos: Vector2, target_node, dmg: int, spd: float, team_name: String, is_homing: bool = true, splash: float = 0.0, p_piercing: bool = false, p_max_range: float = 0.0, p_pierce_radius: float = 0.0, p_source_unit_id: String = "", p_source_hit_event_id: String = "") -> void:
	position = spawn_pos
	target = target_node
	damage = dmg
	speed = spd
	...
	source_hit_event_id = p_source_hit_event_id
...
## 只为发射该投射物的单位播放专属命中音，避免污染通用 projectile_hit。
func _play_source_impact_sfx() -> void:
	if source_hit_event_id != "":
		AudioManager.play(source_hit_event_id, position)
	elif source_unit_id != "":
		AudioManager.play_unit_sfx(source_unit_id, "hit", position)
