...
var _flight_time: float = 0.0  ## 箭矢火焰的本地视觉计时，不参与伤害或同步
...
## 箭矢外观飞行绘制。appearance=arrow 仅由公主使用：五支箭在高抛途中保持燃烧尾焰。
...
## 绘制连续燃烧尾迹：外焰、内焰、摇曳火舌和飞散余烬均由飞行时间驱动。
## 仅使用本地视觉时间，不影响投射物位置、伤害或联机判定。
func _draw_burning_trail(tail: Vector2, tangent: Vector2, alpha: float, arrow_index: float) -> void:
	var perp := Vector2(-tangent.y, tangent.x)
	...
	for flame_index in range(3):
		...
		var anchor := tail - tangent * distance
	...
	# 少量余烬持续从羽尾脱落；正弦相位保证不依赖随机数而保持两端稳定。
	for ember_index in range(2):
		var ember_phase := phase + float(ember_index) * 2.6
