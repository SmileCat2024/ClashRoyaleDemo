...
var _front_scale_mult: float = 1.0         ## front 方向额外缩放乘数（默认1.0=不区分，>1 front放大）
var _unit_id: String = ""
...
func setup(unit_data: Dictionary, entity: CombatantBase) -> void:
	...
	_front_scale_mult = float(anim_data.get("front_scale_mult", 1.0))
...
func _play_if_exists(anim_name: String) -> bool:
	...
	# front 方向额外缩放（部分单位 front 帧比 back 帧小，需放大补偿）
	_refresh_direction_scale(anim_name)
	return true


## 按当前动画名是否为 front 方向，叠加方向缩放乘数到 base_scale。
func _refresh_direction_scale(anim_name: String = "") -> void:
	if anim_name == "":
		anim_name = _current_state
	var mult := _front_scale_mult if anim_name.findn("_front") >= 0 else 1.0
	_sprite.scale = Vector2(_base_scale.x * mult, _base_scale.y * mult)
...
func set_deploy_scale(s: Vector2) -> void:
	_deploy_scale = s
	if _sprite:
		var mult := _front_scale_mult if _current_state.findn("_front") >= 0 else 1.0
		_sprite.scale = Vector2(_base_scale.x * s.x * mult, _base_scale.y * s.y * mult)
