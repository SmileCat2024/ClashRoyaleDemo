...
## [版本待定] - 2026-07-16 — 修复索敌穿透隐身（锁定不丢失）

### 修复
- **已锁定目标进入隐身后丢失锁定**：`AttackComponent.has_valid_target()` 此前只校验目标的存在/死亡/ground-air 兼容，未检查 `is_stealthed`。导致单位（如火枪手）锁定显形的皇室幽灵后，幽灵进入隐身（移动/待机）仍被判定为有效目标，`_update_targeting()` 持续保持锁定，出现"穿透隐身继续追打"的违和表现。补齐隐身过滤，与 `TargetingSystem.find_best_target` / `find_nearest_enemy_unit` 的索敌阶段过滤口径保持一致：目标一旦隐身即失效，下一帧重新索敌（隐身单位同样被跳过）。已抬手未结算的攻击若中途目标隐身，结算时自然落空（不执行命中），符合"瞄准中目标消失则攻击打空"的预期。

### 测试
- `test_attack_targeting.gd` 新增 3 个回归用例：锁定目标隐身后丢失锁定（无其他敌人→null）、锁定目标隐身后切换到视野内最近的显形敌人、显形目标不受隐身过滤影响（反向回归）。
...
## [版本待定] - 2026-07-16 — 对局结算规则统一

### 修改

- **国王塔即时结算**：任意阶段任一方国王塔被摧毁，立刻结束比赛；被摧毁的一方失败。
- **加时赛触发与猝死**：常规时间结束时，双方剩余皇冠塔数量相同才进入加时赛；加时期间任一国王塔或公主塔被摧毁，立刻判该方失败。
- **加时最终判定**：加时结束仍未分胜负时，只比较双方国王塔与两座公主塔中“当前血量最低”的一座；最低血量更高的一方获胜，完全相同则平局。
- **结算表现**：战斗结束先保留短暂的最后一击反馈，随后在画面中央显示本地玩家视角的 `VICTORY`、`DEFEAT` 或 `DRAW`。
- **测试**：新增结算回归测试，覆盖常规时间平局进入加时、国王塔即时结束、加时任意皇冠塔猝死和最低塔血量判定。

## [版本待定] - 2026-07-16 — 精英重甲亡灵死亡标志敌我区分 + 切换干脆化

### 改进
- **死亡标志敌我颜色区分**：精英重甲亡灵「死亡俯冲」被动跟踪的血量最低目标脚下处决标志（`DeathMark`），现在按释放方阵营带轻微色调倾向——我方（player）施加的标志稍微泛蓝（`COLOR_PLAYER_TINT` 深蓝黑），敌方（enemy）施加的稍微泛红（`COLOR_ENEMY_TINT` 深红黑），主体仍为深色，便于一眼区分是谁施加的标记。`spawn_persistent` 新增 `team` 参数，`UnitBase._update_passive_mark` 传入 `self.team`。
- **标志切换干脆化**：被动标志目标切换（或技能释放/单位死亡）时，旧标志不再走 0.4 秒渐隐，`expire()` 改为直接 `queue_free()`，新标志随后立即出现，实现干脆的切换效果。同时清理 `_fading`/`_fade_elapsed`/`FADE_DURATION` 死代码，并修正定时模式（`spawn`）`_process` 未累加 `_elapsed` 导致永不自动销毁的遗留缺陷（该模式当前未被调用）。

## [版本待定] - 2026-07-16 — 精英骑士圣光嘲讽
...
- **神箭游侠**：模型缩放从 0.023 微调至 0.025，血条同步上移；移除底部 ColorRect 调试占位。
- **公主**：高抛溅射箭矢弹速修正为 **10.0 格/秒**，弧高由 5.0 提升为 **7.0 格**；背身和正面攻击均在第 3 帧后放箭。5 支箭使用轻盈的半透明浅红/浅黄燃烧尾迹，飞行中逐步向落点展开但不越出 AOE。
- **重甲亡灵**：三方向攻击帧时长约放慢 30%，使出爪与收势更清晰。
- **神箭游侠**：穿透攻击由激光式路径带改为半透明浅蓝实体长箭；先显形、再脱手飞行，抵达最大射程后箭尾没入终点。伤害判定改为当前箭身，不再命中已飞过路径上的目标。
- **神箭游侠移动**：正反两向行走帧从每帧 0.15 秒放慢至 **0.24 秒**，使步态更从容。
- **神箭游侠箭矢**：进一步拉长至 **6.5 格**、加宽至 **14 像素**；移除箭头尖端，以多层低透明度蓝色柔化边缘，避免生硬高亮。
- **预设卡组**：第 4 套「迫击炮阵地」以神箭游侠替换哥布林。
...
- **远程单位接近大型建筑后原地僵住**：统一 `TargetingSystem` 的视野判定与攻击触及距离几何口径。索敌现在按“视野 + 攻击者碰撞半径 + 目标受击半径”计算实体边缘距离，保证单位在 `UnitBase` 的攻击停步距离内必能锁定目标。火枪手在桥头过河后可正常锁定并攻击敌方公主塔；同类远程单位和大型目标也不再出现停步死区。
- **移除公主的数值绕过**：公主的视野从 13.0 格还原为与其攻击射程一致的 10.5 格；此前为规避同一系统缺陷而额外增加的视野不再需要。
- 新增索敌和 AttackComponent 回归测试，覆盖“自身 0.5 格碰撞半径 + 公主塔 1.5 格受击半径”下的 160px 攻击边界。
...
### 新增
- **觉醒牌系统**：卡牌打出指定次数（trigger_count）后，下一次打出为觉醒版（强化效果）。循环模式——觉醒版打完后重新累计。示例包括骑士护盾/血量强化和女枪远程狙击弹。
- **精英牌系统**：已有牌可附带主动技能。骑士的集结号角提供自身狂暴；重甲亡灵的死亡俯冲锁定残血单位并造成范围伤害。

### 修改
- 新增 `AwakeningTracker`、`EliteSkillManager`、技能栏与按钮、死亡标记、狙击弹视觉，以及对应的测试套件。
- 出牌链路、卡槽视觉、HUD、数据校验、单位状态机均已接入觉醒与精英技能。

## [0.21.1] - 2026-07-13 — 修复联机 Client 选牌能量检查误用房主能量

### 修复
- **联机 Client 选牌能量检查误用房主能量**：本地选牌拦截改为读取显示空间的 `"player"` 能量；实际 Host 出牌链路和部署位置逻辑保持不变。

### 修改
- `scripts/battle/BattleManager.gd`：`_select_hand_card` 能量检查改用 `can_afford_card("player", …)`。
